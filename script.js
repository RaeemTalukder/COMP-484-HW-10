...
function clickedTreatButton() {
  //log functions to log when treat is on cooldown, is already being played, or doesn't have enough energy
   logGroup("Treat action", () => {
    if (isBusy()) logWarning("Pet is busy");
    if (isOnCooldown("treat")) logWarning("Treat on cooldown");
    if (!hasEnergy(1)) logError("Not enough energy");


  if (isBusy() || isOnCooldown("treat") || !hasEnergy(1)) return;
    logInfo("Treat applied");
    logTable(pet_info);
...
function clickedPlayButton() {
  logGroup("Play action", () => {
   if (isBusy()) logWarning("Pet is busy");
    if (isOnCooldown("play")) logWarning("Play on cooldown");
    if (!hasEnergy(1)) logError("Not enough energy");
  if (isBusy() || isOnCooldown("play") || !hasEnergy(2)) return;

   logInfo("play applied");
    logTable(pet_info);
...
function clickedExerciseButton() {
  logGroup("exercise action", () => {
    if (isBusy()) logWarning("Pet is busy");
    if (isOnCooldown("exercise")) logWarning("exercise on cooldown");
    if (!hasEnergy(1)) logError("Not enough energy");
